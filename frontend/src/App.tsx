...
// Sidebar items ordered by business flow (Roadmap)
// ── Flow operativo: Aziende → Lead → Opportunità → Preventivi → Approvazioni → Contratti → Fatture → Progetti → Ore → Ticket → Documenti
// ── Analytics & Config: Report → Notifiche → Prodotti → Reparti → Template → Utenti → Registro
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <div key={item.path}>
              ...
              <a href={item.path}
                className={`flex items-center gap-3 px-4 py-2 text-sm transition-colors mx-2 rounded-lg ${isActive ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}
              >
                <SvgIcon d={item.icon} />
...
function KPICard({ label, value, sub, color, loading }: { label: string; value: string; sub: string; color: string; loading?: boolean }) {
  ...
  return (
    <div className={`rounded-xl border p-5 ${colors[color] || colors.blue}`}>
      ...
      {loading ? (
        <div className="h-9 bg-current opacity-10 rounded mt-1 animate-pulse" />
      ) : (
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Panoramica operativa CRM Connecteed</p>
        </div>
        {error && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">CRM Connecteed</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {[
              ...
              <a key={a.label} href={a.href} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
...
function AuthLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex bg-gray-50" style={{ height: '125vh' }}>
      <Sidebar />
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white border-b border-gray-200 h-14 flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
            <a href="/notifications" className="relative text-gray-500 hover:text-gray-700">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
            </a>
            <button onClick={() => { localStorage.removeItem('access_token'); localStorage.removeItem('refresh_token'); window.location.href = '/login'; }} className="text-xs text-gray-500 hover:text-red-600">Logout</button>
          </div>
        </header>
        {children}
      </main>
